...
String albmnamee=request.getParameter("albmName");
...
<!-- 音乐界面--------------------------------------->
	<div id="musicdetail" style="width: 60%; margin: 0 auto; border-left:solid 2px #E5E5E5;border-right:solid 2px #E5E5E5 ;background: #EDEDED">
      ...
      <img v-bind:src="simgurl" class="media-object"  style="padding-left:20px;width: 175px;height: 177px; border:2px solid white" alt="封面" >
		...
		<p style="padding-left: 25px;color: gray;">简介：{{sintro}}</p>
			<p> </p>
		...
		<div id="serdetail" style="width: 70%; margin: 0 auto; padding-top:222px; border-left:solid 2px #E5E5E5;border-right:solid 2px #E5E5E5 ;background: #EDEDED" >
			<div class="panel panel-default">
				<h3 style="padding-left: 10px">曲目列表</h3>
		...
		<table class="table">
		        ...
		        <tbody>
		          <tr v-for="(item,index) in songbyalbm":key="item.sId">
		...
		<script type="text/javascript">
			...
			let ser=new Vue({
				...
				created: function () {
			    	...
			    	this.getSongWithAlbm(1,albmname);
			    },
			    methods:{
			    	//跳转歌曲详情页
			    	...
			    	//查询相关歌手信息
			    	getSongWithAlbm:function(id,name){
			    		var albmdet = this;
			    		$.ajax({
			    			url:"${APP_path}/albmname",
			    			data: {"pnumber":id,"albmName":name},
			    			type:'get',
			    			dataType: 'json',
			    		}).then(function(res){
			    			console.log(res);
			    			albmdet.songbyalbm=res.extend.pageInfo.list;	
			    			albmdet.ser=res.extend.pageInfo.list[0].serName;
			    			albmdet.sintro=res.extend.pageInfo.list[0].sIntro;
			    			albmdet.simgurl=res.extend.pageInfo.list[0].sImgUrl;
			    		}).fail(function(res){
			    			console.log('失败');
			    		})
			    	}
